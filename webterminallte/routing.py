...
channel_routing = [
    route_class(Webterminal, path=r'^/ws/(?P<ip>(?:(?:0|1[\d]{0,2}|2(?:[0-4]\d?|5[0-5]?|[6-9])?|[3-9]\d?)\.){3}(?:0|1[\d]{0,2}|2(?:[0-4]\d?|5[0-5]?|[6-9])?|[3-9]\d?))/(?P<id>[\w]+)/$'),
    route_class(SshTerminalMonitor, path=r'^/monitor/(?P<channel>[\w-]+)/(?P<id>[\w]+)/$'),
]
